# ---------- Version A: Exact same file, only page_eda() replaced ----------
"""
...
# ============================================================================
# PAGE 2: EXPLORATORY DATA ANALYSIS (REPLACED)
# ============================================================================
...
def page_eda():
    ...
    # Remove ID columns & free-text columns
    drop_cols = [
        'response_id','timestamp','email_capture',
        'open_feedback','other_comments','reason_for_no'
    ]
    ...
    # ----------------------------------------------------------
    # TAB 1 — UNIVARIATE CHARTS
    # ----------------------------------------------------------
    with tab1:
        st.markdown("### 📊 Univariate Distribution Viewer")
        ...
        if col in numeric_cols:
            ...
            fig = px.bar(x=vc.index, y=vc.values, title=f"Count of {col}")
            st.plotly_chart(fig, use_container_width=True)

    # ----------------------------------------------------------
    # TAB 2 — BIVARIATE CHARTS
    # ----------------------------------------------------------
    with tab2:
        st.markdown("### 📉 Relationship Between Two Variables")

        x_var = st.selectbox("X-Axis", df.columns, key="xvar")
        y_var = st.selectbox("Y-Axis", df.columns, key="yvar")

        if x_var in categorical_cols and y_var in numeric_cols:
            fig = px.box(df, x=x_var, y=y_var,
                         title=f"{y_var} Across {x_var}")
            st.plotly_chart(fig, use_container_width=True)

        elif x_var in numeric_cols and y_var in numeric_cols:
            ...
            fig = px.imshow(ct, text_auto=True,
                            title=f"Cross Tab: {x_var} vs {y_var}")
            st.plotly_chart(fig, use_container_width=True)

        else:
            st.info("⚠️ Select valid combination.")
    ...
    with tab3:
        ...
        # ===== B. Cramer's V (categorical vs categorical) =====
        st.markdown("### 🔗 Cramér’s V (Categorical Correlations)")
        ...
        cat_pairs = pd.DataFrame(index=categorical_cols, columns=categorical_cols)

        for c1 in categorical_cols:
            for c2 in categorical_cols:
                try:
                    cat_pairs.loc[c1, c2] = cramers_v(df[c1], df[c2])
                except:
                    cat_pairs.loc[c1, c2] = None

        fig = px.imshow(cat_pairs.astype(float),
                        color_continuous_scale="Purples",
                        title="Cramér’s V Heatmap (Categorical Correlation)",
                        aspect="auto")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        if "shopizz_monthly_spending" in df.columns:
            fig = px.box(df, x="age", y="shopizz_monthly_spending",
                         title="Spending by Age Group")
            st.plotly_chart(fig, use_container_width=True)

            fig = px.box(df, x="shopping_style", y="shopizz_monthly_spending",
                         title="Spending by Shopping Style")
            st.plotly_chart(fig, use_container_width=True)

        st.markdown("### 👀 Sustainability vs Artisan Support")
        if "sustainability_importance" in df.columns and "artisan_support" in df.columns:
            ct = pd.crosstab(df["sustainability_importance"], df["artisan_support"])
            fig = px.imshow(ct, text_auto=True,
                            title="Sustainability vs Artisan Support")
            st.plotly_chart(fig, use_container_width=True)
